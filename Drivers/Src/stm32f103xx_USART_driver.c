...
void USART_Init(USART_Handle_s *USART_Handle_p) {
	...
	if (USART_Handle_p->USARTx_p == USART1) {
		//USART1 and USART6 are hanging on APB2 bus
		PCLKx_u32 = RCC_GetPCLK2Value();
	} else {
		PCLKx_u32 = RCC_GetPCLK1Value();
	...
	if (USART_Handle_p->USART_Config.USART_HwFlowControl
			== USART_HW_FLOW_CTRL_CTS) {
		//Implement the code to enable CTS flow control 
		TempReg_u32 |= (1 << USART_CR3_CTSE_POS);
	} else if (USART_Handle_p->USART_Config.USART_HwFlowControl
...
//	USART_SetBaudRate(USART_Handle_p->USARTx_p, USART_Handle_p->USART_Config.USART_BaudRate);
	USART_Handle_p->USARTx_p->BRR = UART_BRR_SAMPLING16(PCLKx_u32, USART_Handle_p->USART_Config.USART_BaudRate);

	USART_Handle_p->USARTx_p->CR2 &= ~(1 << USART_CR2_LINEN_POS);
	USART_Handle_p->USARTx_p->CR2 &= ~(1 << USART_CR2_CLKEN_POS);

	USART_Handle_p->USARTx_p->CR3 &= ~(1 << USART_CR3_SCEN_POS);
	USART_Handle_p->USARTx_p->CR3 &= ~(1 << USART_CR3_IREN_POS);
	USART_Handle_p->USARTx_p->CR3 &= ~(1 << USART_CR3_HDSEL_POS);
} /* END of USART_Init */
...
void USART_Tx(USART_Handle_s *USART_Handle_p, uint8_t *TxBuffer_p, uint32_t Len_u32) {
	...
	if ((USART_Handle_p->USART_Config.USART_WordLength == USART_WORDLEN_9BITS)
			&& (USART_Handle_p->USART_Config.USART_ParityControl == USART_PARITY_DISABLE)) {
		Data8Bits_p = NULL;
		Data16Bits_p = (uint16_t*) TxBuffer_p;
	} else {
		Data8Bits_p = TxBuffer_p;
		Data16Bits_p = NULL;
	}
	...
	while (Len_u32 > 0)
	{
		...
		if (Data8Bits_p == NULL) {
			...
			USART_Handle_p->USARTx_p->DR = (uint16_t) (*Data16Bits_p & 0x01FFu);
			Data16Bits_p++;
		} else {
			//This is 8bit data transfer 
			USART_Handle_p->USARTx_p->DR = (uint8_t) (*Data8Bits_p & 0xFFu);
...
void USART_Rx(USART_Handle_s *USART_Handle_p, uint8_t *RxBuffer_p,
		uint32_t Len_u32) {
	//Loop over until "Len" number of bytes are transferred
	for (uint32_t i = 0; i < Len_u32; i++) {
		//Implement the code to wait until RXNE flag is set in the SR
		while (RESET
				== READ_BIT(USART_Handle_p->USARTx_p->SR, USART_SR_RXNE_POS)) {
			;
		}
		...
		if (USART_Handle_p->USART_Config.USART_WordLength == USART_WORDLEN_9BITS) {
			...
			if (USART_Handle_p->USART_Config.USART_ParityControl
					== USART_PARITY_DISABLE) {
				//No parity is used. so, all 9bits will be of user data

				//read only first 9 bits. so, mask the DR with 0x01FF
				*((uint16_t*) RxBuffer_p) = (USART_Handle_p->USARTx_p->DR
						& (uint16_t) 0x01FF);
			...
			if (USART_Handle_p->USART_Config.USART_ParityControl
				...
				//read 8 bits from DR
				*RxBuffer_p = (uint8_t) (USART_Handle_p->USARTx_p->DR
						& (uint8_t) 0xFF);
			} else {
 ...
 */
uint8_t USART_TxIT(USART_Handle_s *USART_Handle_p, uint8_t *TxBuffer_p,
		uint32_t Len_u32) {
...
uint8_t USART_RxIT(USART_Handle_s *USART_Handle_p, uint8_t *RxBuffer_p,
		uint32_t Len_u32) {
...
void USART_IRQ_Priority(uint8_t IRQ_Number_u8, uint8_t IRQ_Priority_u8) {
	...
	uint8_t ShiftAmount_u8 = (IPRx_Section_u8 * 8u)
			+ (8u - NUM_PR_BITS_IMPLEMENTED); // Last 4 bits are not implemented

	*(NVIC_PR_BASE_ADDR + IPRx_u8) |= (uint32_t) (IRQ_Priority_u8
			<< ShiftAmount_u8);
